...
public class SectionController : ControllerBase
{
    ...
    public SectionController(AppDbContext db) => _db = db;
    ...
    public async Task<ActionResult<SectionResponseDto>> Create([FromBody] SectionCreateDto dto)
    {
        ...
        var classroom = await _db.Classrooms.FindAsync(dto.ClassroomId);
    ...
    public async Task<ActionResult<SectionResponseDto>> Update(Guid id, [FromBody] SectionUpdateDto dto)
    {
        ...
        {
            var instructor = await _db.Users.FindAsync(dto.InstructorId.Value);
            if (instructor is null || instructor.Role != UserRole.Instructor)
                return BadRequest(new { message = "Instructor not found or user is not an Instructor." });
        }
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        var hasEnrollments = await _db.Enrollments.AnyAsync(e => e.SectionId == id);
        if (hasEnrollments)
            return Conflict(new
            {
    ...
    /// </summary>
    internal async Task<bool> HasInstructorConflictAsync(
    ...
    /// </summary>
    internal static bool SharesDay(string a, string b) =>
        a.Any(c => b.Contains(c, StringComparison.OrdinalIgnoreCase));

    // internal so tests can verify mapping logic directly
    internal static SectionResponseDto MapToResponse(Section s) => new()
    {
        Id = s.Id,
        CourseId = s.CourseId,
